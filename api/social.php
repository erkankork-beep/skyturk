...
function sky_make_card(array $n, string $outPath): bool {
  ...
  $S=1080; $im=imagecreatetruecolor($S,$S); $navy=imagecolorallocate($im,11,42,107); $navy2=imagecolorallocate($im,20,33,61); $white=imagecolorallocate($im,255,255,255); $red=imagecolorallocate($im,232,38,44); $yel=imagecolorallocate($im,255,212,0); $sky=imagecolorallocate($im,120,200,255); $grey=imagecolorallocate($im,200,210,230);
  imagefilledrectangle($im,0,0,$S,$S,$navy2);
  $F=__DIR__.'/lib/font/unifont/'; $fT=$F.'DejaVuSansCondensed-Bold.ttf'; $fB=$F.'DejaVuSans.ttf'; $fBB=$F.'DejaVuSans-Bold.ttf';
  /* fotoğraf (üst %58) */
  $ph=(int)($S*0.58); $img=null;
  ...
  /* alt blok */
  imagefilledrectangle($im,0,$ph,$S,$S,$navy2); imagefilledrectangle($im,0,$ph,$S,6,$red);
  /* kategori etiketi */
  ...
  $title=trim($n['t']??''); $maxW=$S-96; $size=54; do{ $lines=[]; $cur=''; foreach(preg_split('/\s+/u',$title) as $wd){ $t=$cur===''?$wd:$cur.' '.$wd; $b=imagettfbbox($size,0,$fT,$t); if(($b[2]-$b[0])<=$maxW) $cur=$t; else { if($cur!=='') $lines[]=$cur; $cur=$wd; } } if($cur!=='') $lines[]=$cur; if(count($lines)<=4) break; $size-=4; }while($size>=34);
  $lines=array_slice($lines,0,4); $lh=(int)($size*1.22); $y=$ph+34+48+28+$size;
  foreach($lines as $l){ imagettftext($im,$size,0,48,$y,$white,$fT,$l); $y+=$lh; }
  /* alt imza şeridi */
  imagefilledrectangle($im,0,$S-64,$S,$S,$navy); imagettftext($im,28,0,48,$S-22,$white,$fT,'SKY'); $bw=imagettfbbox(28,0,$fT,'SKY'); imagettftext($im,28,0,48+($bw[2]-$bw[0]),$S-22,$sky,$fT,'TÜRK');
  $sig=trUpS(($n['d']??'')?mb_substr($n['d'],0,10):date('d.m.Y')).'  ·  skyturk_haber'; $sb=imagettfbbox(18,0,$fB,$sig); imagettftext($im,18,0,$S-48-($sb[2]-$sb[0]),$S-24,$grey,$fB,$sig);
  imagefilledellipse($im,48+($bw[2]-$bw[0])+imagettfbbox(28,0,$fT,'TÜRK')[2]+14,$S-30,12,12,$red);
