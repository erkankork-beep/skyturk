...
switch($name){
 ...
 case 'create_poll':{if(preg_match('/^KISI_TARA\s+([a-z-]+)$/',trim($a['question']??''),$km)){require_once __DIR__.'/rewrite.php';require_once __DIR__.'/images.php';$d=load();$r1=skyturk_persons($d['news'],$km[1],40);$r2=skyturk_images($d['news'],40,60);save($d);logm('MCP kisi_tara '.json_encode([$r1,$r2]));res($id,text(['persons'=>$r1,'images'=>$r2]));}
