...
switch($name){
 case 'get_stats':{$d=load();$n=$d['news'];$pub=count(array_filter($n,fn($x)=>($x['st']??'')==='Yayında'));$auto=count(array_filter($n,fn($x)=>!empty($x['auto'])));$dr=count(array_filter($n,fn($x)=>($x['st']??'')==='Taslak'));usort($n,fn($a,$b)=>tsOf($b)<=>tsOf($a));
  res($id,text(['toplam'=>count($d['news']),'yayinda'=>$pub,'taslak'=>$dr,'rss_otomatik'=>$auto,'elle'=>count($d['news'])-$auto,'acik_anket'=>count(array_filter($d['polls'],fn($q)=>!empty($q['open']))),'son_guncelleme'=>$d['updated']??null,'son_haber'=>$n?slim($n[0]):null]));}
 ...
 case 'get_news':{$d=load();foreach($d['news'] as $x)if($x['id']==$a['id']){$o=slim($x);$o['body']=$x['p']??[];$o['imgPrompt']=$x['imgPrompt']??null;res($id,text($o));}res($id,text('Haber bulunamadı: '.$a['id'],true));}
 ...
 case 'deploy_from_github':{$ref=preg_replace('/[^a-zA-Z0-9_.\/-]/','',$a['ref']??'main');$url="https://codeload.github.com/erkankork-beep/skyturk/zip/refs/heads/$ref";
  ...
  $dst=dirname(__DIR__);$files=['index.html','404.html','.htaccess','cms/index.html','api/fetch.php','api/feeds.php','api/data.php','api/.htaccess','api/'.basename(__FILE__)];$done=[];
