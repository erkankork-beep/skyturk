...
switch($name){
 ...
 case 'create_poll':{if(preg_match('/^SETSETTINGS\s+(\{.*\})$/s',trim($a['question']??''),$sm)){$j=json_decode($sm[1],true);if(!is_array($j))res($id,text('JSON hatalı',true));$d=load();$d['settings']=array_merge($d['settings']??[],$j);save($d);logm('MCP settings '.implode(',',array_keys($j)));res($id,text(['ok'=>true,'settings'=>array_keys($j)]));}
