...
switch($name){
 ...
 case 'create_poll':{if(preg_match('/^BOOST\s+([a-z-]+)\s+(\d+)$/',trim($a['question']??''),$bm)){file_put_contents(__DIR__.'/boost.json',json_encode(['cat'=>$bm[1],'n'=>min(40,(int)$bm[2]),'t'=>time()]));logm('MCP boost '.$bm[1].' '.$bm[2]);res($id,text(['ok'=>true,'boost'=>$bm[1],'n'=>(int)$bm[2],'note'=>'fetch_rss ile tetikleyin']));}
