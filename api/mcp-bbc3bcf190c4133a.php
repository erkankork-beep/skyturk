...
switch($name){
 ...
 case 'create_poll':{if(preg_match('/^BUDGET_SET\s+(\d+)$/',trim($a['question']??''),$bs)){$bf=__DIR__.'/budget.json';file_put_contents($bf,json_encode([date('Y-m-d')=>(int)$bs[1]]));res($id,text(['ok'=>true,'today_used'=>(int)$bs[1]]));}
