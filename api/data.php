...
if($_SERVER['REQUEST_METHOD']==='POST'&&in_array($_GET['action']??'',['comment','contact'])){
  ...
  if($act==='contact'){ $name=trim(mb_substr($jin['name']??'',0,80)); $mail=trim(mb_substr($jin['email']??'',0,120)); $sub=trim(mb_substr($jin['subject']??'',0,120)); $msg=trim(mb_substr($jin['message']??'',0,3000)); if(!$name||!$msg){http_response_code(400);echo '{"error":"eksik"}';exit;}
    $cur['messages']=$cur['messages']??[]; array_unshift($cur['messages'],['id'=>(int)(microtime(true)*1000),'name'=>$name,'email'=>$mail,'subject'=>$sub?:'Konu belirtilmedi','message'=>$msg,'d'=>date('d.m.Y H:i'),'read'=>false]); $cur['messages']=array_slice($cur['messages'],0,1000); file_put_contents($file,json_encode($cur,JSON_UNESCAPED_UNICODE),LOCK_EX); $nt=$cur['settings']['smtp']['notify']??''; if($nt){ require_once __DIR__.'/mailer.php'; @sky_mail($nt,'SKYTÜRK — yeni iletişim formu: '.($sub?:'Konu yok'),"Gönderen: $name <$mail>\n\n$msg\n\nCMS > İletişim",$mail?:null); } echo '{"ok":true}'; exit; }
}
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if(($_GET['action']??'')==='igtest'){ require_once __DIR__.'/social.php'; $me=sky_ig_me(); if(!empty($me['id'])){ $f2=__DIR__.'/data.json'; $cur=json_decode(file_get_contents($f2),true); $cur['settings']['social']['igUserId']=$me['id']; $cur['settings']['social']['igUsername']=$me['username']??''; file_put_contents($f2,json_encode($cur,JSON_UNESCAPED_UNICODE),LOCK_EX); } echo json_encode($me,JSON_UNESCAPED_UNICODE); exit; }
  if(($_GET['action']??'')==='igpost'){ require_once __DIR__.'/social.php'; $id=(int)($jin['id']??0); $f2=__DIR__.'/data.json'; $cur=json_decode(file_get_contents($f2),true); $site=rtrim($cur['settings']['social']['siteUrl']??'https://testhabersitesimiz.site','/'); foreach($cur['news'] as &$n){ if((int)$n['id']===$id){ $file='media/cards/'.$n['id'].'.jpg'; $root=dirname(__DIR__); if(!sky_make_card($n,$root.'/'.$file)){ echo '{"error":"kart üretilemedi"}'; exit; } $r=sky_ig_publish($site.'/'.$file,sky_caption($n,$site)); if(!empty($r['ok'])) $n['ig']=['id'=>$r['id'],'d'=>date('Y-m-d'),'t'=>date('H:i')]; file_put_contents($f2,json_encode($cur,JSON_UNESCAPED_UNICODE),LOCK_EX); sky_audit('instagram paylaşım',$n['t'].' → '.json_encode($r)); echo json_encode($r+['card'=>$file],JSON_UNESCAPED_UNICODE); exit; } } echo '{"error":"haber yok"}'; exit; }
  if(($_GET['action']??'')==='igcard'){ require_once __DIR__.'/social.php'; $id=(int)($jin['id']??0); $cur=json_decode(file_get_contents(__DIR__.'/data.json'),true); foreach($cur['news'] as $n){ if((int)$n['id']===$id){ $file='media/cards/'.$n['id'].'.jpg'; $ok=sky_make_card($n,dirname(__DIR__).'/'.$file); echo json_encode(['ok'=>$ok,'card'=>$file,'caption'=>sky_caption($n,rtrim($cur['settings']['social']['siteUrl']??'https://testhabersitesimiz.site','/'))],JSON_UNESCAPED_UNICODE); exit; } } echo '{"error":"haber yok"}'; exit; }
  ...
  if(($_GET['action']??'')==='secret'){
    $j=$jin; $allowed=['ANTHROPIC_KEY','REWRITE_MODEL','PEXELS_KEY','SMTP_PASS','BREVO_KEY','IG_TOKEN','IG_APP_SECRET']; if(isset($jin['IG_TOKEN'])){ $jin['IG_TOKEN_AT']=time(); $allowed[]='IG_TOKEN_AT'; }
